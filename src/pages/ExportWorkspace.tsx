...
import { IconDownload, IconCheck, IconSparkles } from "../components/Icons";
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between p-4 rounded-lg bg-[#18191d] border border-[#2a2b31] gap-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {done && (
            <div className="flex items-center gap-1.5 text-emerald-400 text-xs font-semibold">
              <IconCheck size={14} />
              <span>EXPORT BUNDLE READY ({projectBasename}.zip)</span>
            </div>
          )}
        </div>
        ...
        {done && (
          <a
            ...
            className="px-4 py-2 rounded bg-cyan-500/20 text-cyan-400 border border-cyan-500/40 text-xs font-semibold hover:bg-cyan-500/30 transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
            <IconDownload size={14} />
            <span>DOWNLOAD 3D GLB FILE</span>
          </a>
        )}
      </div>
    </div>
